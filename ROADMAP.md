...
- 이직률 모델 ✅ (`lib/labor.ts` attritionRate·applyAttrition, `decisions.salaryMultiplier` 0.7~1.5, <1.0 시 자연 이탈 최대 20%, 다음 분기 headcount 자동 감소)
...
- 2026-04-24 (5) 엔진 최종 마감 5종 (176/176 테스트, store v16):
  ...
  - **3 경쟁사 archetype** — 3사 각각 고유 "공격형 리더·가성비형 추격자·수비형 로컬" 설명·강점·플레이북, results 프로파일에 노출
  - **8 Phase D 보강** (`lib/tax.ts`) — 투자세액공제 `§24 7%` + DTA/DTL 순효과 재분류 + 15년 만료 헬퍼, BS에 `deferredTaxAsset` 필드
  - **9 이직률 모델** — `salaryMultiplier` 0.7~1.5 레버, <1.0 시 자연 이탈 (최대 20%), 각 부서 최소 1명 floor, advanceRound에서 decisions.headcount 자동 감소·lastAttrition 경고 표시
